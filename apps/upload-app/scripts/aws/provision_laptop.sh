...
aws --profile "$PROFILE" iam create-user --user-name "$IAM_USER" \
    --tags "${TAGS[@]}" "Key=managed_by,Value=upload-app-provision-laptop" >/dev/null
...
UPLOAD_S3_PREFIX=${S3_PREFIX}
CREDS
chmod 600 "$OUT"

if [[ -n "$HOST" ]]; then
    next="next: send that file to the owner of $LAPTOP_ID privately (1Password shared group, or AirDrop). They save it next to
  fc_sync.sh and run:  bash fc_sync.sh start   - it moves the key into place. Delete your copy once they confirm.
  Their ~/.env_data_ingestion_apps needs DATA_DIR (the folder to upload; the other apps use the same line) and
  UPLOAD_S3_BUCKET=${BUCKET} and UPLOAD_S3_PREFIX=${S3_PREFIX} from it for the manifest URIs."
else
    next="next: bash scripts/fc_sync.sh start"
fi
cat <<EOT
