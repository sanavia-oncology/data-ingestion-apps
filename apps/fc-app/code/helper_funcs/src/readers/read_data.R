...
read_data = function(paths, verbose=FALSE) {
    ...
    merged_data_path = paths[grep("merged-data.csv$", paths)]
    mfi_channel_path = paths[grep("mfi-channel.csv$", paths)]
    ...
    ch = mfi_channel$mfi_channel[1]
    merged_data$mfi_channel = sapply(strsplit(ch, "> "), "[[", 2)
